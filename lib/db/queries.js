...
module.exports = {
  ...
  setPlayerAvailability: `
    ...
    update set available = $2, answered = now()
    returning
      player_id, available, day
  `,
  fetchPlayerAvailability: `
    select player_id, available, day
    from player_availability
    where player_id = $1
    and day = ($2 AT TIME ZONE '${config.timezone}'::date)
  `,
  playerStats: `
    with north_matches as (
            select dmo.doubles_match_id, dmo.north_wins, p.id as player_id, p.first_name, dmo.updated_at
            from
                    teams t,
                    players p,
    ...
    south_matches as (
            select dmo.doubles_match_id, dmo.south_wins, p.id as player_id, p.first_name, dmo.updated_at
            from
                    teams t,
                    players p,
                    season_league_players slp,
    ...
    matches as (
            select player_id, doubles_match_id, north_wins as wins, updated_at from north_matches
            union all
            select player_id, doubles_match_id, south_wins as wins, updated_at from south_matches
    )
    ...
    select
            ...
            max(matches.updated_at) as last_played
    from players p
  ...
  fetchTeams: `
        select
                t.*, p1.id as player1_id, p2.id as player2_id
        from teams t
        join season_league_players slp1 on slp1.id = t.season_league_player1_id
        join season_league_players slp2 on slp2.id = t.season_league_player2_id
        join players p1 on slp1.player_id = p1.id
        join players p2 on slp2.player_id = p2.id
        join seasons s on slp1.season_id = s.id
        where s.active = 't'
  `
}